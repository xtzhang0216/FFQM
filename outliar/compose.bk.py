...
def get_charmm(pdb_file):
    # pdb_file = "/pubhome/xtzhang/interaction/FF/mol2/ACEM.pdb"
    psf = ForceField('/pubhome/xtzhang/interaction/FF/charmm/charmm36.zxt.fake.xml','charmm36/water.xml')
    pdb = PDBFile(pdb_file)
    # for res in pdb.topology.residues():
    #     print(f"PDB Residue: {res.name}")
    system = psf.createSystem(
...
# model = "charmm"
# pair = "ACET_PRPA"
sdfpath = f"/pubhome/xtzhang/interaction/FF/outliar/outliar_head/{pair}/{pair}_outliar.charmm.sdf"
...
output_file = Path(f'/pubhome/xtzhang/interaction/FF/outliar/outliar_head/{pair}/{pair}.{model}.csv')
