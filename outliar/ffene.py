...
cwd = Path('/pubhome/xtzhang/interaction/FF/cgenff/ChgParam_byzlj')
# cwd = Path(__file__).absolute().parent
# vdw_params_f = cwd / 'ffnonbonded.itp'
# path = /pubhome/xtzhang/interaction/FF/scripts/charmm36-jul2022.ff
vdw_params_f = Path("/pubhome/xtzhang/interaction/FF/cgenff/charmm36-jul2022.ff/ffnonbonded.itp")
    ...
    for sdffile in sdffiles:
        ...
        with Chem.SDMolSupplier(sdffile, removeHs=False) as suppl:
            for idx, mol in enumerate(suppl):
                ...
                if idx == 30:
                    pass
                if ff_ene - qm_ene > 5 and  qm_ene < 0:
                    results[str(idx)] = [round(qm_ene, 2), round(ff_ene, 2), round((ff_ene - qm_ene), 2), round(vdw_pair, 2), round(chg_pair, 2)]
        # Save results to a CSV file
        results_df = pd.DataFrame.from_dict(results, orient='index', columns=['QM_Energy', 'FF_Energy', 'diff', 'vdw', 'chg'])
        results_df.index.name = 'Index'
        output_file = Path(f'/pubhome/xtzhang/interaction/FF/outliar/outliar_head/{pair}.csv')
